...
+!cfp(OrderId, Customer, Table, Dish, Priority, Reputation)[source(restaurantManager)] :
        available(waiter1, true) & completedTasks(C) & failedTasks(F) & distance(Table, D) <-
    Score = D + F * 6 - C + (100 - Reputation) / 10;
    .print("waiter1: bid score=", Score, " distance=", D, " priority=", Priority);
    .send(restaurantManager, achieve, propose(OrderId, waiter1, Score)).

+!cfp(OrderId, Customer, Table, Dish, Priority, Reputation)[source(restaurantManager)] :
        available(waiter1, false) <-
    .send(restaurantManager, achieve, propose(OrderId, waiter1, 999)).

+!acceptOrder(OrderId, Customer, Table, Dish, Priority)[source(restaurantManager)] :
        available(waiter1, true) <-
    .print("waiter1: accepted ", OrderId, " priority=", Priority);
    -available(waiter1, true);
    +available(waiter1, false);
    +pendingOrder(OrderId, Customer, Table, Dish, Priority);
    .send(restaurantManager, achieve, orderAccepted(OrderId, waiter1));
    !goTo(waiter1, Table);
    .send(inventoryManager, achieve, checkDish(Dish, waiter1, OrderId)).

+!acceptOrder(OrderId, Customer, Table, Dish, Priority)[source(restaurantManager)] :
        available(waiter1, false) <-
    .print("waiter1: cannot accept ", OrderId, " because another task is still active");
    .send(restaurantManager, achieve, orderDeclined(OrderId, waiter1)).

+!rejectOrder(OrderId)[source(restaurantManager)] : true <-
    .print("waiter1: ", OrderId, " assigned to another waiter.").
...
+!dishAvailable(Dish, OrderId)[source(inventoryManager)] :
    ...
    .print("waiter1: requesting adaptive chef bids for ", Dish);
    .send(chef1, achieve, cfpCook(OrderId, Customer, Dish, Priority, waiter1));
    .send(chef2, achieve, cfpCook(OrderId, Customer, Dish, Priority, waiter1));
    .wait(800);
    !chooseChef(OrderId).

+!dishUnavailable(Dish, Missing, none, OrderId)[source(inventoryManager)] :
        pendingOrder(OrderId, Customer, Table, Dish, Priority) <-
    .print("waiter1: ", Dish, " unavailable; no feasible alternative. Missing=", Missing);
    setTableOrder(Table, Dish, unavailable);
    .send(Customer, tell, orderUnavailable(Customer, Dish, Missing));
    -pendingOrder(OrderId, Customer, Table, Dish, Priority);
    -available(waiter1, false);
    +available(waiter1, true);
    .send(restaurantManager, achieve, waiterOutcome(waiter1, failure));
    .send(restaurantManager, achieve, waiterFree(waiter1));
    !goTo(waiter1, start).
...
+!dishUnavailable(Dish, Missing, AltDish, OrderId)[source(inventoryManager)] :
    ...
    .print("waiter1: proposing ", AltDish, " because ", Dish, " is unavailable; missing ", Missing);
    +pendingAlternative(OrderId, stock, Dish, AltDish);
    setTableOrder(Table, Dish, awaiting_customer);
    .send(Customer, achieve,
          considerAlternative(OrderId, Dish, AltDish, stock, Missing, waiter1)).
...
+!alternativeAccepted(OrderId, OriginalDish, AltDish, stock)[source(Customer)] :
    ...
    .print("waiter1: customer accepted stock alternative ", AltDish);
    setTableOrder(Table, AltDish, alternative);
    .send(inventoryManager, achieve, checkDish(AltDish, waiter1, OrderId)).
...
+!chefAlternativeProposal(OrderId, Chef, OldDish, AltDish)[source(Chef)] :
    ...
    .print("waiter1: asking ", Customer, " to approve chef re-plan ", OldDish, " -> ", AltDish);
    .send(Customer, achieve,
          considerAlternative(OrderId, OldDish, AltDish, replan, [resource_conflict], waiter1)).
...
+!alternativeAccepted(OrderId, OldDish, AltDish, replan)[source(Customer)] :
    ...
    .print("waiter1: customer approved chef re-plan to ", AltDish);
...
+!alternativeRejected(OrderId, OldDish, AltDish, replan)[source(Customer)] :
        ...
        pendingOrder(OrderId, Customer, Table, OriginalDish, Priority) <-
    -pendingAlternative(OrderId, replan, OldDish, AltDish);
    .print("waiter1: customer rejected chef alternative ", AltDish);
...
+!chefBid(OrderId, Chef, Score)[source(Chef)] :
    ...
    .print("waiter1: updated chef bid ", Score, " from ", Chef).

+!chefBid(OrderId, Chef, Score)[source(Chef)] :
        selectingChef(OrderId) & not chefOffer(OrderId, Chef, OldScore) <-
    +chefOffer(OrderId, Chef, Score);
    .print("waiter1: chef bid ", Score, " from ", Chef).

+!chefBid(OrderId, Chef, Score)[source(Chef)] : not selectingChef(OrderId) <-
    .print("waiter1: ignoring late chef bid from ", Chef).
...
+!chooseChef(OrderId) :
    ...
    .print("waiter1: selecting chef1 score=", B1);
    setTableOrder(Table, Dish, assigned);
    .send(chef1, achieve, cookAccepted(OrderId, Customer, Dish, Priority, waiter1));
...
+!chooseChef(OrderId) :
    ...
    .print("waiter1: selecting chef2 score=", B2);
    setTableOrder(Table, Dish, assigned);
    .send(chef2, achieve, cookAccepted(OrderId, Customer, Dish, Priority, waiter1));
...
+!chooseChef(OrderId) : selectingChef(OrderId) & pendingOrder(OrderId, Customer, Table, Dish, Priority) <-
    .print("waiter1: no chef available; reopening negotiation for ", OrderId);
...
+!deliverOrder(OrderId, Customer, Table, OriginalDish, Dish, Chef) :
    ...
    .print("waiter1: collecting ", Dish, " from ", Chef);
    !goTo(waiter1, kitchen);
    pickupDish(Dish);
    !goTo(waiter1, Table);
    ...
    +available(waiter1, true);
    -+completedTasks(C + 1);
    .send(restaurantManager, achieve, waiterOutcome(waiter1, success));
    .send(restaurantManager, achieve, waiterFree(waiter1));
    !goTo(waiter1, start).
...
-!deliverOrder(OrderId, Customer, Table, OriginalDish, Dish, Chef) :
    ...
    -+failedTasks(F + 1);
    .print("waiter1: delivery intention failed; re-planning ", OrderId);
    .send(Customer, tell, coordinationDelay(Customer, delivery_replan));
    .send(restaurantManager, achieve, waiterOutcome(waiter1, failure));
    .wait(1000);
    .send(inventoryManager, achieve, checkDish(OriginalDish, waiter1, OrderId)).
...
+!handleSupply(Customer, Table, Item)[source(restaurantManager)] : available(waiter1, true) <-
    -available(waiter1, true);
    +available(waiter1, false);
    +supplyTaskActive(Customer, Table, Item);
    !deliverSupplyTask(Customer, Table, Item).

+!handleSupply(Customer, Table, Item)[source(restaurantManager)] : available(waiter1, false) <-
    .print("waiter1: supply task deferred because another local task is active");
    .send(restaurantManager, achieve, supplyDeferred(waiter1, Customer, Table, Item)).

+!cancelSupply(Customer, Table, Item)[source(restaurantManager)] :
        supplyTaskActive(Customer, Table, Item) <-
    -supplyTaskActive(Customer, Table, Item);
    +supplyCancelledByManager(Customer, Table, Item);
    .print("waiter1: cancelling stale ", Item, " task for ", Customer).

+!cancelSupply(Customer, Table, Item)[source(restaurantManager)] :
        not supplyTaskActive(Customer, Table, Item) <- true.

+!deliverSupplyTask(Customer, Table, Item) : supplyTaskActive(Customer, Table, Item) <-
    .send(inventoryManager, achieve, checkSupply(Item, waiter1, Customer, Table)).
...
+!supplyOk(Item, Customer, Table)[source(inventoryManager)] :
    ...
    -available(waiter1, false);
    +available(waiter1, true);
    .send(restaurantManager, achieve, supplyCancelled(waiter1, Customer, Table, Item));
    .send(restaurantManager, achieve, waiterFree(waiter1));
    !goTo(waiter1, start).
...
+!completeSupplyDelivery(Customer, Table, Item, C) :
    ...
    .send(restaurantManager, achieve,
          supplyCompleted(waiter1, Customer, Table, Item));
    .send(restaurantManager, achieve, waiterOutcome(waiter1, success));
    .send(restaurantManager, achieve, waiterFree(waiter1));
    !goTo(waiter1, start).
...
-!completeSupplyDelivery(Customer, Table, Item, C) :
    ...
    -available(waiter1, false);
    +available(waiter1, true);
    .print("waiter1: returned cancelled ", Item, " to storage");
    .send(restaurantManager, achieve,
          supplyCancelled(waiter1, Customer, Table, Item));
    .send(restaurantManager, achieve, waiterFree(waiter1));
    !goTo(waiter1, start).
...
+!cleanTableTask(Table)[source(restaurantManager)] : available(waiter1, true) <-
    -available(waiter1, true);
    +available(waiter1, false);
    .print("waiter1: going to clean ", Table);
    !goTo(waiter1, Table);
    .wait(1400);
    reclaimReusableSupply(Table, cutlery);
    cleanTable(Table, waiter1);
    .print("waiter1: finished cleaning ", Table);
    .send(restaurantManager, achieve, tableCleaned(waiter1, Table));
    -available(waiter1, false);
    +available(waiter1, true);
    .send(restaurantManager, achieve, waiterFree(waiter1));
    !goTo(waiter1, start).

+!cleanTableTask(Table)[source(restaurantManager)] : available(waiter1, false) <-
    .print("waiter1: cleaning task deferred because waiter is busy");
    .send(restaurantManager, achieve, cleaningDeferred(waiter1, Table)).

+!goTo(waiter1, Loc) : true <- move(waiter1, Loc).
